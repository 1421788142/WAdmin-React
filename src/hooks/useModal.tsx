...
export function useModalDragMove(context: useModalInterface) {
    ...
    const drag = (wrap: any) => {
        ...
        wrap.setAttribute('data-drag', context.draggable); //添加属性 是否打开了model框

        const modelHeaderEl = wrap.querySelector('.ant-modal-header');//获取拖动dom

        const modelDom = wrap.querySelector('.ant-modal');//获取需要移动dom

        if (!modelHeaderEl || !modelDom || !context.draggable) return; //拖动dom || 没有需要拖到的dom || 和是否打开容器
    ...
    if (!context.open || !context.destroyOnClose) {
        return;
    }
    setTimeout(() => {
        handleDrag()
